...
def test_sage():
  ...
  part_ptr, part_to_nodes = ext_forward.build_part(part_size, torch.IntTensor(row_pointers))
  ...
  out = ext_forward.sage_forward(g.ndata["x"], weight, torch.IntTensor(row_pointers).cuda(), torch.IntTensor(column_index).cuda(), 

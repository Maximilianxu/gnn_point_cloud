...
std::tuple<std::vector<std::vector<vint>>, std::map<int, int>> rabbit_reorder(
  std::vector<std::vector<vint>>& edge_index
) {
  int src, dst;
  ...
  const int num_edges = edge_index[0].size();
  // const int dim0 = in_edge_index.size(1);
  // vint is size_t 
  std::vector<std::tuple<vint, vint, float>> edges(num_edges, std::make_tuple(0, 0, 1.0f));
  // prepare the input
  for(int i = 0; i < num_edges; i++){
    src = edge_index[0][i];
    dst = edge_index[1][i];
    edges[i] = std::make_tuple(src, dst, 1.0f);
  }
  ...
  std::vector<std::vector<vint>> out_edge_index(2, std::vector<vint>(num_edges, 0));
  ...
  for(int i = 0; i < num_edges; i++){
    src = std::get<0>(edges[i]);
    dst = std::get<1>(edges[i]);
    out_edge_index[0][i] = mapping[src];
    out_edge_index[1][i] = mapping[dst];
  }

  return std::make_tuple(out_edge_index, mapping);
}

void test_torch(torch::Tensor &t){
